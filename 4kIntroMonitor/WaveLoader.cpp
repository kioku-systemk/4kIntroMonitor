...
#define fillMem(Destination,Length,Fill) opt_memset((Destination),(Fill),(Length))
...
int WaveLoader::SetWavePointerMS(DWORD dwMS, int method){
	if( isLoaded ){
		m_dwCurrentDecodedPos = kftol(dwMS/1000.0 * (double)m_wfx.nAvgBytesPerSec);
		mmioSeek(m_hmmio, kftol((dwMS/1000.0 * (double)m_wfx.nAvgBytesPerSec) + m_dwOffsetToWaveData), method);
	}
	return CSL_E_OK;
}