...
void main()
{
	float j = 1.0 - cos(t*10.0);
	sphere[0].center = vec3(-2.0, 0.0, -3.5);
	sphere[0].radius = 0.5;
	sphere[1].center = vec3(-0.5, j*0.5, -3.0);
	sphere[1].radius = 0.5;
	...
	vec3 org = vec3(0,0,5.0 - sin(t)*5.0);
	vec2 p = (gl_FragCoord.xy-vec2(w/2.0,h/2.0))/vec2(w,h);
	vec3 dir = normalize(-vec3(-p.x*1.6,-p.y,1));
